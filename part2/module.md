...
앵귤러 앱은 모듈로 구성되며 앵귤러는 NgModules 이라는 자체 모듈 시스템을 사용합니다. JavaScript에서는 각각의 파일 이 하나의 모듈이며, 이 파일에 정의된 모든 객체가 이 모듈 안에 속한다고 할 수 있습니다. 반면에 앵귤러의 모듈은 사용자가 모듈에 등록한 구성요소들만이 해당 모듈이 갖고 있는 자원이 됩니다.

모든 앵귤러 앱에는 적어도 하나의 NgModule 클래스가 존재합니다. 처음 시작하는 모듈을 루트 모듈이라고 부릅니다. 루트 모듈은 작은 응용 프로그램에서 유일한 모듈 일 수 있지만 대부분의 응용 프로그램은 많은 기능 모듈을 필요로 합니다. NgModule 클래스는 `@NgModule` 데코레이터가 있는 클래스입니다. `@NgModule` 데코레이터는 모듈을 설정하는 정보를 가진 메타 데이터 객체를 취하는 데코레이터 함수입니다. 설정 정보를 가진 객체에서 가장 중요한 속성은 다음과 같습니다.
